...
namespace jrmwng
{
	...
	struct dct_matrix
	{
		...
		dct_matrix()
		{
			for_each(std::make_index_sequence<uN>(), [&](auto const tN)
			{
				for_each(std::make_index_sequence<uN>(), [&, tN](auto const tQ)
				{
	...
	void dct(T const (&atInput)[uN], T(&atOutput)[uN])
	{
		...
		for_each(std::make_index_sequence<uN>(), [&](auto const k)
		{
			T tAcc(0);
			{
				for_each(std::make_index_sequence<uN>(), [&, k](auto const n)
				{
	...
	void dct(__m256d const (&alr4Input)[uN4], __m256d(&alr4Output)[uN4])
	{
		...
		for_each(std::make_index_sequence<uN4>(), [&](auto const k4)
		{
			__m256d lr4Acc = _mm256_setzero_pd();
			{
				for_each(std::make_index_sequence<uN4>(), [&](auto const n4)
				{
					__m256d const lr4InputABCD = alr4Input[n4];

					for_each(std::make_index_sequence<4>(), [&](auto const n1)
					{
	...
	void dct(__m256 const (&ar8Input)[uN8], __m256(&ar8Output)[uN8])
	{
		...
		for_each(std::make_index_sequence<uN8>(), [&](auto const k8)
		{
			__m256 r8Acc = _mm256_setzero_ps();
			{
				for_each(std::make_index_sequence<uN8>(), [&](auto const n8)
				{
					__m256 const r8InputABCDEFGH = ar8Input[n8];

					for_each(std::make_integer_sequence<unsigned, 8>(), [&](auto const n1)
					{
	...
	void dct(__m128d const (&alr2Input)[uN2], __m128d(&alr2Output)[uN2])
	{
		...
		for_each(std::make_index_sequence<uN2>(), [&](auto const k2)
		{
			__m128d lr2Acc = _mm_setzero_pd();
			{
				for_each(std::make_index_sequence<uN2>(), [&](auto const n2)
				{
					__m128d const lr2InputAB = alr2Input[n2];

					for_each(std::make_index_sequence<2>(), [&](auto const n1)
					{
